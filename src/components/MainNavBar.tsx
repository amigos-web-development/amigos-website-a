...
const MainNavBar = () => {
  ...
  return (
    <>
      {/* Blur overlay when products menu is open */}
      {productsOpen && (
        <div
      ...
      <nav className="bg-nav-bg border-b border-nav-border sticky top-0 z-40 shadow-sm">
        ...
        {productsOpen && (
          <div className="hidden lg:block absolute left-0 w-full bg-card border-b border-border shadow-xl z-50 animate-scale-in">
              ...
              <div className="grid grid-cols-4 gap-3">
                {productCategories.map((cat) => (
                  <a
                    key={cat.label}
                    href="#"
                    className="flex items-center gap-3 px-5 py-4 text-sm text-foreground hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors"
                  >
                    <cat.icon className="w-5 h-5 flex-shrink-0 text-muted-foreground" />
                    {cat.label}
                  </a>
                ))}
              </div>
